...
void LogShots::Paint()
{
    ...
    for (auto ev : LogShots::eventList)
    {
        ...
        if (ev.expTime > globalVars->curtime && delta < 6.f)
        {
            ...
            Draw::AddText(8, offset, ev.string.c_str(), ImColor(255, 255, 255));
            offset += 12;
            i++;
        }
        else
...
void LogShots::FireGameEvent(IGameEvent *event)
{
    ...
    if (!strcmp(event->GetName(), XORSTR("player_hurt")))
    {
        ...
        cvar->ConsoleColorPrintf(ColorRGBA(39, 106, 219, 255), XORSTR("[Zedeware] "));
...
void LogShots::CreateMove(CUserCmd* cmd)
{
    ...
    if (tr.m_pEntityHit == shot.ent) // resolver error
    {
        ...
        {
            case 0:
                str = XORSTR("Missed shot due to brain issue\n");
                break;
            case 1:
                str = XORSTR("Missed shot due to anti-hit\n");
                break;
            case 2:
                str = XORSTR("Missed shot due to xd\n");
                break;
            case 3:
                str = XORSTR("Missed shot due to pasted resolver\n");
                break;
            case 4:
                str = XORSTR("Missed shot due to spoonlae moonlae\n");
                break;
        }
